...
### 5. Document Exploration & Issues

Steps 4 and 5 happen together -- explore and document in a single pass.

**Track exploration progress** as you go through the app. Keep a running list of:
- ✅ Areas explored (Dashboard, Workflows, Data Hub, etc.)
- Features tested within each area
- Console errors observed (or note if none)
- Notable technical observations

**When you find an issue**, stop exploring and document it immediately before moving on. Do not explore the whole app first and document later.
...
1. **Add Details section to report** (append after Issues section):
...
✅ **Dashboard**
- Yield Excursion Workflow table
- Search/filtering functionality
- Pagination controls
- Agent selection

✅ **Workflows Section**
- Workflows tab (5 workflows with various statuses)
- Runs tab (workflow execution history)
- Schedules tab

... (continue for all areas tested)

### Technical Notes

- No JavaScript console errors detected throughout the session
- All core navigation and interactive features functioned as expected
- Form validation working correctly
- Responsive UI elements (dropdowns, buttons, search) all operational
...
- Report generated: {OUTPUT_DIR}/report.md
```

2. **Create Jira bugs** for each issue found:
   - Use `/create-bug` skill for each ISSUE-NNN
   - Provide issue summary from report, actual/expected behavior, and screenshot path
   - Add the Jira issue ID to the report under each issue (e.g., `**Jira:** [SEMI-1234](https://emergenceai.atlassian.net/browse/SEMI-1234)`)

3. Re-read the report and update the summary severity counts so they match the actual issues. Every `### ISSUE-` block must be reflected in the totals.
...
5. Tell the user the report is ready and summarize findings: total issues, breakdown by severity, Jira bugs created, and the most critical items.
...
- **Always add exploration details to the report.** After testing is complete, append a Details section with areas explored, technical notes, and session metadata. This provides context for stakeholders reviewing the report.
- **Create Jira bugs automatically.** For every issue found (ISSUE-001, ISSUE-002, etc.), invoke the `/create-bug` skill to create a Jira ticket. Link the created Jira issue back to the report for traceability.
